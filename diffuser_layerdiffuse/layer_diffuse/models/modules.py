...
class TransparentVAEDecoder(AutoencoderKL):
    ...
    def estimate_augmented(self, pixel, latent):
        ...
        # Batch all 8 augmentations into a single forward pass
        feed_pixels = []
        feed_latents = []
        for flip, rok in args:
            ...
            fl = latent.clone()
            if flip:
                fp = torch.flip(fp, dims=(3,))
                fl = torch.flip(fl, dims=(3,))
            fp = torch.rot90(fp, k=rok, dims=(2, 3))
            fl = torch.rot90(fl, k=rok, dims=(2, 3))
            feed_pixels.append(fp)
            feed_latents.append(fl)

        batch_pixel = torch.cat(feed_pixels, dim=0)
        batch_latent = torch.cat(feed_latents, dim=0)
        batch_eps = self.estimate_single_pass(batch_pixel, batch_latent).clip(0, 1)

        result = []
        for i, (flip, rok) in enumerate(args):
            eps = batch_eps[i:i+1]
